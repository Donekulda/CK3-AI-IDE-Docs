...
PixelShader =
{
	...
	MainCode PixelShader
	{
		...
		[[
			...
			PDX_MAIN
			{
				// Note[DB]: We could move these into a constant-buffer so they can be set from defines/code/gui script?
				// Tweak the variable here for effect
				...
				// We're not using the `PopOutThreshold` for pop-out anymore, so we never pop-out.
				// Code kept here for if we want to re-add that
 				if ( false /* Input.UV1.y < PopOutThreshold */ )
 				{
